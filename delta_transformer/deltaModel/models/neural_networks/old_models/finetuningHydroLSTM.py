...
class FineTunerHydro(nn.Module):
    def __init__(self, config: Union[Dict, DictConfig], ny) -> None:
        ...
        # Initialize decoder using CudnnLstm instead of nn.LSTM
        ...
        log.info("Successfully initialized FineTuner model with CudnnLstm decoder")
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if isinstance(self.adapter, nn.Identity):
                ...
                if isinstance(self.adapter, nn.Sequential) and isinstance(self.adapter[0], nn.Conv1d):
                    ...
                    adapted = self.adapter(torch.cat([hidden_states, batch_x], dim=-1))
            
            # Convert from [batch, seq, features] to [seq, batch, features] for CudnnLstm
            adapted = adapted.permute(1, 0, 2)
            
            # Decode with CudnnLstm
            dec_output, (hn, cn) = self.decoder(
                adapted, 
                do_drop_mc=False, 
                dr_false=(not self.training)
            )
            
            # Convert back to [batch, seq, features] for projection
            dec_output = dec_output.permute(1, 0, 2)
            
            # Final projection
            outputs = self.projection(dec_output)
            
            # Ensure output shape consistency - return as [time, batch, params]
...
class GatedAdapter(nn.Module):
    ...
    def forward(self, combined: torch.Tensor) -> torch.Tensor:
        # Combined input already has hidden states and input features concatenated
        gate = torch.sigmoid(self.gate(combined))
        transformed = self.transform(combined)
        # Extract just the hidden states portion from the combined input
        h = combined[..., :self.gate.out_features]
