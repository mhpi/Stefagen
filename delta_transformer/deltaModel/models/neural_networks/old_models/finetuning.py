...
class FineTuner(nn.Module):
    ...
    # @staticmethod
    # def debug_tensor(tensor: torch.Tensor, name: str = "tensor") -> torch.Tensor:
    #     """Static helper method to debug tensor states with safe CPU checking.
        
    #     Parameters
    #     ----------
    #     tensor : torch.Tensor
    #         Input tensor to debug
    #     name : str
    ...
    #     -------
    #     torch.Tensor
    #         Input tensor, potentially with NaN values replaced
    #     """
    #     try:
    #         # Move tensor to CPU for safer checking
    ...
    #             with torch.no_grad():
    #                 stats = {
    #                     'min': cpu_tensor[~torch.isnan(cpu_tensor)].min().item() if (~torch.isnan(cpu_tensor)).any() else 'all NaN',
    #                     'max': cpu_tensor[~torch.isnan(cpu_tensor)].max().item() if (~torch.isnan(cpu_tensor)).any() else 'all NaN',
    #                     'mean': cpu_tensor[~torch.isnan(cpu_tensor)].mean().item() if (~torch.isnan(cpu_tensor)).any() else 'all NaN'
    #                 }
    #             log.info(f"{name} stats - min: {stats['min']}, max: {stats['max']}, mean: {stats['mean']}")
                
    #             # Replace NaN values with 0 on the original device
    #             tensor = torch.nan_to_num(tensor, nan=0.0)
                
    #         return tensor
            
    #     except Exception as e:
    #         log.warning(f"Error in debug_tensor for {name}: {str(e)}")
    #         return tensor  # Return original tensor if debugging fails
    # def forward(self, data_dict: Dict[str, torch.Tensor], eval_mode: bool = False, eval: bool = False) -> Dict[str, torch.Tensor]:
        # """Forward pass with separate paths for training and evaluation."""
        # if eval_mode:
        #     print("eval_mode")
        #     return self._forward_eval(data_dict)
