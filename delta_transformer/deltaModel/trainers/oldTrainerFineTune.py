...
class FineTuneTrainer(BaseTrainer):
    ...
    def create_loss(self) -> torch.nn.Module:
        ...
        loss_name = self.config.get('criterion', 'NSE')
        if loss_name == 'NSE':
            return NSELoss()
        elif loss_name == 'Rmse':
            return RMSELoss()
            ...
            for epoch in range(self.start_epoch, self.config['train']['epochs'] + 1):
               ...
               # Get grid dimensions for training
                ...
                for i in range(1, n_minibatch + 1):
                    ...
                    batch_data = self._get_batch_data(n_samples, n_timesteps)
    ...
    def test(self) -> None:
        ...
        batch_size = min(self.config['test']['batch_size'], total_basins)
                ...
                for i in range(n_batches):
                    ...
                    try:
                        ...
                        if isinstance(prediction, dict):
                            if 'HBV' in prediction and 'flow_sim' in prediction['HBV']:
                                # Get the prediction and remove the warm-up period
                                batch_pred = prediction['HBV']['flow_sim'].cpu().numpy()
    ...
    def _get_time_window_sample(self, dataset, basin_start, basin_end, time_start, time_end):
        ...
        return batch_data
    def calc_metrics(self, predictions: np.ndarray, observations: torch.Tensor) -> None:
        """Calculate and log comprehensive model performance metrics."""
        # Ensure observations is numpy array
        if torch.is_tensor(observations):
            observations = observations.cpu().numpy()
        
        # If predictions and observations have different shapes, adjust
        if predictions.ndim != observations.ndim:
            # Typical case: predictions might be [batches, time, 1] while observations might be [time, batches, 1]
            if len(predictions.shape) == 3 and len(observations.shape) == 3:
                # Try to match dimensions
                if predictions.shape[0] == observations.shape[1] and predictions.shape[1] == observations.shape[0]:
                    observations = np.transpose(observations, (1, 0, 2))
                
        # Log basic comparison stats
        log.info("Prediction vs Observation Statistics:")
        log.info(f"Predictions shape: {predictions.shape}")
        log.info(f"Observations shape: {observations.shape}")
        
        # Basic statistical comparison
        log.info("\nStatistical Comparison:")
        log.info(f"Prediction Mean: {np.mean(predictions):.4f}")
        log.info(f"Observation Mean: {np.mean(observations):.4f}")
        log.info(f"Prediction Std Dev: {np.std(predictions):.4f}")
        log.info(f"Observation Std Dev: {np.std(observations):.4f}")
        ...
        try:
            ...
            metrics_file = os.path.join(
                self.config.get('save_path', 'results'), 
                'test_metrics.txt'
            )
    ...
    # def _get_validation_batch(self, start: int, end: int) -> Dict[str, torch.Tensor]:
    #     """Get a batch of validation/test data."""
    #     batch_data = {
    #         key: value.float().to(self.device)
    #         for key, value in self.sampler.get_validation_sample(
    #             self.test_dataset,
    #             start,
    #             end
    #         ).items()
    ...
    # def _create_results_plots(self, predictions: np.ndarray, observations: np.ndarray) -> None:
    #     """Create and save visualization plots."""
    #     results_dir = self.config.get('save_path', 'results')
    #     os.makedirs(results_dir, exist_ok=True)
        
    #     fig, ax = plt.subplots(figsize=(10, 6))
    #     ax.scatter(observations.flatten(), predictions.flatten(), alpha=0.5)
    #     ax.plot([0, observations.max()], [0, observations.max()], 'r--')
