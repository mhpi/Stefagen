...
class TestModeTypeEnum(str, Enum):
    temporal = 'temporal'
    spatial = 'spatial'


class ExtentEnum(str, Enum):
    PUR = 'PUR'
    PUB = 'PUB'


class TestModeConfig(BaseModel):
    type: TestModeTypeEnum = Field(..., description="Type of test mode (temporal or spatial)")
    gage_split_file: Optional[str] = Field(None, description="Path to the gage split file")
    extent: Optional[ExtentEnum] = Field(None, description="Spatial extent (PUR or PUB)")
    holdout_indexs: Optional[List[int]] = Field(None, description="List of indices to hold out based on region config")
    huc_regions: Optional[List[List[int]]] = Field(None, description="HUC region configuration for PUR")
    PUB_ids: Optional[List[int]] = Field(None, description="PUB IDs for region configuration")

    @model_validator(mode='after')
    def validate_spatial_config(cls, values):
        """Validates configuration for spatial test mode"""
        if values.type == TestModeTypeEnum.spatial:
            if values.extent is None:
                raise ValueError("'extent' must be specified when test_mode.type is 'spatial'")
            if values.holdout_indexs is None or not values.holdout_indexs:
                raise ValueError("'holdout_indexs' must be specified when test_mode.type is 'spatial'")
            
            if values.extent == ExtentEnum.PUR and values.huc_regions is None:
                raise ValueError("'huc_regions' must be specified when extent is 'PUR'")
            
            if values.extent == ExtentEnum.PUB and values.PUB_ids is None:
                raise ValueError("'PUB_ids' must be specified when extent is 'PUB'")
            
            if values.gage_split_file is not None:
                try:
                    check_path(values.gage_split_file)
                except ValueError as e:
                    raise ValueError(f"Invalid gage_split_file: {e}")
